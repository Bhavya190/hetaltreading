...
export default function DailySaleDetailPage() {
  ...
  const saleId = params?.id ? decodeURIComponent(params.id as string) : ''

  const [sale, setSale] = useState<DailySaleRecord | null>(null)
  ...
  const fetchSaleDetail = async () => {
    ...
    try {
      setLoading(true)
      const res = await fetch(`/api/daily-sales/${encodeURIComponent(saleId)}`)
      ...
      console.error('Error fetching sale bill detail:', err)
      setSale(null)
    } finally {
      setLoading(false)
    }
  ...
  if (loading) {
    return (
      <div className="p-12 text-center text-slate-500 flex flex-col items-center justify-center gap-3">
        ...
        <p className="text-sm font-semibold">Loading sales bill invoice...</p>
      </div>
    )
  }
  ...
  const gstSummary = calculateBillGstTotals(sale.items || [], sale.extraCharges || 0)
      ...
      <div className="bg-white rounded-2xl p-6 border border-slate-200 shadow-xs space-y-6">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-slate-200 pb-5">
          <div className="space-y-1.5">
            <div className="inline-flex items-center gap-1.5 text-blue-900 text-[11px] font-extrabold uppercase tracking-wider bg-blue-50 px-2.5 py-0.5 rounded border border-blue-200 font-mono">
              ...
              <span>GST TAX INVOICE #: {sale.billNumber}</span>
            </div>
            <h1 className="text-2xl font-extrabold text-slate-900 pt-1">{sale.customerName}</h1>
            <div className="flex items-center gap-4 text-xs text-slate-500 font-medium">
              <div className="flex items-center gap-1 font-mono">
                <Calendar className="w-3.5 h-3.5 text-slate-400" />
                <span>Date: {sale.date ? sale.date.split('T')[0] : ''}</span>
              </div>
            </div>
          </div>
        ...
        <div className="space-y-4">
          <h2 className="font-extrabold text-xs text-slate-700 uppercase tracking-wider flex items-center gap-2">
            ...
            <span>Product Line Items with GST Breakdown ({gstSummary.processedItems.length})</span>
          </h2>
